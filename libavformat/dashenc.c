                                ...
                                int representation_id, int final)
{
    ...
    // put "?" at the front so we can easily enable/disable it
    ...
    if (c->use_template) {
        ...
        avio_printf(out, "initialization=\"%s%s\" media=\"%s%s\" startNumber=\"%d\">\n", os->init_seg_name, garbage, os->media_seg_name, garbage, c->use_timeline ? start_number : 1);
        if (c->use_timeline) {
            int64_t cur_time = 0;
